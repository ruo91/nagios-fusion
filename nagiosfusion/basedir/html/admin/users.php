...
function show_users($error=false,$msg=""){
	...
	$sql=generate_sql_query(DB_NAGIOSFUSION,$sql_args);
...
<?php
	...
	$sql=limit_sql_query_records($sql_args,$cfg['db_info'][DB_NAGIOSFUSION]['dbtype']);
	$rs=exec_sql_query(DB_NAGIOSFUSION,$sql);
	...
	if(!$rs || $rs->EOF){
		...
		if(have_value($classes))
			echo " class='".$classes."'";
		echo ">";
		echo "<td><input type='checkbox' name='user_id[]' value='".encode_form_val($oid)."' id='checkbox_".encode_form_val($oid)."' ".$checked."></td>";
		echo "<td class='clickable'>".htmlentities($rs->fields["username"])."</td>";
		echo "<td class='clickable'>".htmlentities($rs->fields["name"])."</td>";
		echo "<td class='clickable'><a href='mailto:".urlencode($rs->fields["email"])."'>".htmlentities($rs->fields["email"])."</a></td>";
		echo "<td>";
		echo "<a href='?edit=1&amp;user_id[]=".urlencode($oid)."'><img class='tableItemButton' src='".theme_image("b_edituser.png")."' border='0' alt='".$lstr['EditAlt']."' title='".$lstr['EditAlt']."'></a> ";
		echo "<a href='?clone=1&amp;user_id[]=".urlencode($oid)."'><img class='tableItemButton' src='".theme_image("b_cloneuser.png")."' border='0' alt='".$lstr['CloneAlt']."' title='".$lstr['CloneAlt']."'></a>";
		echo "<a target='_parent' href='?user_id=".urlencode($oid)."&masquerade=1&nsp=".get_nagios_session_protector_id()."' class='masquerade_link'><img class='tableItemButton' src='".theme_image("b_masquser.png")."' border='0' alt='".$lstr['MasqueradeAlt']."' title='".$lstr['MasqueradeAlt']."'></a> ";
		echo "<a href='?delete=1&amp;user_id[]=".urlencode($oid)."&nsp=".get_nagios_session_protector_id()."'><img class='tableItemButton' src='".theme_image("b_deleteuser.png")."' border='0' alt='".$lstr['DeleteAlt']."' title='".$lstr['DeleteAlt']."'></a>";
		echo "</td>";
		echo "</tr>\n";
...
function show_edit_user($error=false,$msg=""){
	...
	if(isset($request["user_id"])){
		$user_id=current($request["user_id"]);
		if($user_id!=0)
			$add=false;
		}
