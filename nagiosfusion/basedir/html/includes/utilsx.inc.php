...
//
// $Id: utilsx.inc.php 75 2010-04-01 19:40:08Z egalstad $
...
function load_url($url,$options=array('method'=>'get','return_info'=>false),$use_proxy=false) {
    ...
    if(function_exists("curl_init") 
        ...
        if(isset($options['method']) and $options['method'] == 'post') {
            $page = $url_parts['scheme'] . '://' . $url_parts['host'];
				    if(!empty($url_parts['port'])){
							$page = $page . ':'. $url_parts['port'];
				    }
            $page = $page . $url_parts['path'];
        } else {
            $page = $url;
        }
	...
	{
		$f = @fopen('/usr/local/nagiosfusion/var/load_url.log','w');
