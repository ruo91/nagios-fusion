...
//
// $Id: ajaxhelper.php 94 2010-04-16 20:52:49Z egalstad $
...
function route_request(){
	...
	switch($cmd){
		...
		// FUSION CORE AJAX
		case "getfusioncoreajax":
			ah_get_fusioncore_ajaxdata();
			break;
...
function ah_get_fusioncore_ajaxdata(){
	...
	$optsarr=json_decode($opts);
	//echo "<br />OPTSARR:<br />\n";
	//var_dump($optsarr->args);
	$fname="fusioncore_ajax_".strval($optsarr->func);
	$args=array();
	if($optsarr->args){
		foreach($optsarr->args as $var => $val){
			$args[strval($var)]=strval($val);
